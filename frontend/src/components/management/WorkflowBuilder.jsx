...
import { Plus, Settings2, Play, Save, CheckCircle, XCircle, Zap } from 'lucide-react';
...
export default function WorkflowBuilder({ workflowId, onBack }) {
  ...
  const handleSave = async () => {
    ...
    try {
        ...
        setTestResult({status: 'success', message: 'Workflow saved as Draft.'});
  ...
  const handleTest = async () => {
      if (!currentWorkflowId) {
          setTestResult({status: 'error', message: 'Save workflow first.'});
          setTimeout(() => setTestResult(null), 3000);
          return;
      }
      setIsTesting(true);
      try {
          ...
          const res = await api.post(`/workflows/${currentWorkflowId}/test`, parsedInput);
          if (res.data.status === 'success') {
             setTestResult({status: 'success', message: 'Test passed! Check console.'});
             console.log(res.data);
          } else {
             setTestResult({status: 'error', message: res.data.message || 'Test failed.'});
          }
      } catch(e) {
          setTestResult({status: 'error', message: e.response?.data?.detail?.message || 'Test failed.'});
      } finally {
          setIsTesting(false);
      }
  };
  ...
  const handleActivate = async () => {
      ...
      try {
          await api.post(`/workflows/${currentWorkflowId}/activate`);
          setTestResult({status: 'success', message: 'Workflow Activated!'});
          setTimeout(() => setTestResult(null), 3000);
      } catch(e) {
          setTestResult({status: 'error', message: e.response?.data?.detail?.message || 'Activation failed.'});
          setTimeout(() => setTestResult(null), 3000);
      }
  };
